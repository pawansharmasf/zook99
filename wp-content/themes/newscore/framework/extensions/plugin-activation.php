	...
	class TGM_Plugin_Activation {
		...
		public function init() {
			...
			$this->strings = array(
				...
				'notice_ask_to_update'           => _n_noop( 'The following plugin needs to be updated to its latest version to ensure maximum compatibility with this theme: %1$s.', 'The following plugins need to be updated to their latest version to ensure maximum compatibility with this theme: %1$s.', 'radium' ),
				...
				'update_link'                   => _n_noop( 'Begin updating plugin', 'Begin updating plugin', 'radium' ),
				'activate_link'                  => _n_noop( 'Begin activating plugin', 'Begin activating plugins', 'radium' ),
				'return'                         => __( 'Return to Required Plugins Installer', 'radium' ),
				'dashboard'                      => __( 'Return to the dashboard', 'radium' ),
				'plugin_activated'               => __( 'Plugin activated successfully.', 'radium' ),
				'activated_successfully'         => __( 'The following plugin was activated successfully:', 'radium' ),
				'complete'                       => __( 'All plugins installed and activated successfully. %1$s', 'radium' ),
				'dismiss'                        => __( 'Dismiss this notice', 'radium' ),
			);
		...
		public function notices() {
			...
			if ( ! empty( $message ) ) {
				...
				foreach ( $message as $type => $plugin_groups ) {
					...
					$rendered .= '<p>' . sprintf( translate_nooped_plural( $this->strings[$type], $count, 'radium' ), $imploded, $count ) . '</p>';
				}

				// Setup variables to determine if action links are needed.
				$show_install_link  = $install_link ? '<a href="' . add_query_arg( 'page', $this->menu, admin_url( 'themes.php' ) ) . '">' . translate_nooped_plural( $this->strings['install_link'], $install_link_count, 'radium' ) . '</a>' : '';
				$show_activate_link = $activate_link ? '<a href="' . add_query_arg( 'page', $this->menu, admin_url( 'themes.php' ) ) . '">' . translate_nooped_plural( $this->strings['activate_link'], $activate_link_count, 'radium' ) . '</a>'  : '';
				$show_update_link  = $update_link ? '<a href="' . add_query_arg( 'page', $this->menu, admin_url( 'themes.php' ) ) . '">' . translate_nooped_plural( $this->strings['update_link'], $update_link_count, 'radium' ) . '</a>' : '';
				...
				if ( ! empty( $this->strings['nag_type'] ) ) {
					add_settings_error( 'radium', 'radium', $rendered, sanitize_html_class( strtolower( $this->strings['nag_type'] ) ) );
				} else {
					add_settings_error( 'radium', 'radium', $rendered, $nag_class );
				}
			}

			// Admin options pages already output settings_errors, so this is to avoid duplication.
			if ( 'options-general' !== $current_screen->parent_base ) {
				settings_errors( 'radium' );
			}
	...
	class TGMPA_List_Table extends WP_List_Table {
		...
		protected function _gather_plugin_data() {
			...
			foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
				...
				$table_data[$i]['type'] = isset( $plugin['required'] ) && $plugin['required'] ? __( 'Required', 'radium' ) : __( 'Recommended', 'radium' );
				...
				if ( ! isset( $installed_plugins[$plugin['file_path']] ) ) {
					...
					$table_data[$i]['status'] = sprintf( '%1$s', __( 'Not Updated', 'radium' ) );
				} else {
					$table_data[$i]['status'] = sprintf( '%1$s', __( 'Active', 'radium' ) );
				}
		...
		public function column_plugin( $item ) {
			...
			if ( ! isset( $installed_plugins[$item['file_path']] ) ) {
				$actions = array(
					'install' => sprintf(
						'<a href="%1$s" title="' . __( 'Install', 'radium' ) . ' %2$s">' . __( 'Install', 'radium' ) . '</a>',
						wp_nonce_url(
							add_query_arg(
								array(
			...
			elseif ( is_plugin_inactive( $item['file_path'] ) ) {
				$actions = array(
					'activate' => sprintf(
						'<a href="%1$s" title="' . __( 'Activate', 'radium' ) . ' %2$s">' . __( 'Activate', 'radium' ) . '</a>',
						add_query_arg(
							array(
								'page'                 => TGM_Plugin_Activation::$instance->menu,
								'plugin'               => $item['slug'],
		...
		public function get_bulk_actions() {
			...
			$actions = array(
				...
				'tgmpa-bulk-update'  => __( 'Update', 'radium' ),
			);
		...
		public function process_bulk_actions() {
			...
			if ( 'tgmpa-bulk-install' === $this->current_action()
				...
				if ( empty( $plugin_installs ) ) {
					echo '<div id="message" class="error"><p>' . __( 'No plugins are available to be installed at this time.', 'radium' ) . '</p></div>';
					return false;
				}
			...
			if ( 'tgmpa-bulk-activate' === $this->current_action() ) {
				...
				if ( empty( $plugins_to_activate ) ) {
					echo '<div id="message" class="error"><p>' . __( 'No plugins are available to be activated at this time.', 'radium' ) . '</p></div>';
					return false;
				}
