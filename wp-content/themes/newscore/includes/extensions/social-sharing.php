...
function radium_single_post_side_share () {
	...
	$comment_count = $queried_post->comment_count;
	...
	$radium_single_get_tweets = radium_single_get_tweets();
	$radium_single_get_facebook_likes = radium_single_get_facebook_likes();
	$radium_single_get_plusones = radium_single_get_plusones();
	$radium_single_get_linkedin_shares = radium_single_get_linkedin_shares();
    ...
    <div id="post-side-share-<?php echo $position; ?>" class="post-side-share position-<?php echo $position; ?>">
        ...
        <div class="clearfix">
            <a class="icon icon-twitter icon_embed_tw" href="http://twitter.com/home/?status=<?php echo urlencode($url . " - " . $title); ?>" title="Twitter" target="blank">
            	<?php if( $position == 'left' || $position == 'right') { ?><span class="count"><?php echo $radium_single_get_tweets; ?></span><?php } ?>
            ...
            <a class="icon icon-facebook icon_embed_fb" href="http://www.facebook.com/sharer.php?u=<?php echo urlencode( $url ); ?>&amp;t=<?php echo urlencode( $title ); ?>" title="Facebook" target="blank">
            	<?php if( $position == 'left' || $position == 'right') { ?><span class="count"><?php echo $radium_single_get_facebook_likes; ?></span><?php } ?>
            ...
            <a class="icon icon-google-plus icon_embed_gplus" href="https://plusone.google.com/_/+1/confirm?hl=en&amp;url=<?php echo urlencode($url); ?>" title="Googleplus" target="blank">
                <?php if( $position == 'left' || $position == 'right') { ?><span class="count"><?php echo $radium_single_get_plusones; ?></span><?php } ?>
            ...
            <a class="icon icon-linkedin icon_embed_linkedin" href="http://www.linkedin.com/shareArticle?mini=true&amp;url=<?php echo urlencode($url); ?>&amp;summary=<?php echo urlencode($title); ?>&amp;source=<?php echo urlencode(home_url()); ?>" title="Linkin" target="blank">
                <?php if( $position == 'left' || $position == 'right') { ?><span class="count"><?php echo $radium_single_get_linkedin_shares; ?></span><?php } ?>
            ...
            <a class="icon icon-comment icon_embed_comments" href="#comment" title="Comments">
            	<?php if( $position == 'left' || $position == 'right') { ?><span class="count"><?php echo $comment_count; ?></span><?php } ?>
            	<span class="share-network"><?php _e('Comments', 'radium'); ?></span>
            </a>

            <?php if( $total > 0 && ($position == 'above' || $position == 'below' || $position == 'above-below') ) { ?><div class="post-total-share"><span class="total-count"><?php echo $total; ?></span><span class="total-title"><?php _e('Shares', 'radium'); ?></span></div><?php } ?>

            <?php if( $comment_count > 0 && ($position == 'above' || $position == 'below' || $position == 'above-below') ) { ?><div class="post-total-comments"><span class="total-count"><?php echo $comment_count; ?></span><span class="total-title"><?php if( $comment_count == 1) { _e('Comment', 'radium'); } else { _e('Comments', 'radium'); } ?></span></div><?php } ?>
 ...
 */
 ...
 function radium_social_account_links() {
	...
	if ($tumblr) {
		...
		$tumblr_url = ($tumblr == '#') ? '#' : 'http://'. $tumblr .'.tumbrl.com/';
