...
if( ! function_exists( 'radium_get_theme_customizer_options' ) && $framework->theme_supports('primary', 'skins') ) {
	function radium_get_theme_customizer_options(){
		...
		*/ 
		...
		if( is_admin() ) radium_add_customizer_section( 'header_tools', __( 'Header Tools', 'radium' ), $options['header_tools_options'], 1 );
		...
		$options['header_options'] = array(
			...
			'header_menu_hover_accent_color' => array(
				...
				'selectors' 	=> array( 
					...
					#main-menu .menu-item-object-category .sub-mega-wrap:not(.single-cat) .sub-menu>li.active,
					#main-menu .menu-item-object-category .sub-mega-wrap:not(.single-cat) .sub-menu>li:hover',
					...
					.main_menu>ul>li>a.open-sub-a:before'
				),
			),
		...
		if( is_admin() ) radium_add_customizer_section( 'header', __( 'Header', 'radium' ), $options['header_options'], 2 );
		...
		if( is_admin() ) radium_add_customizer_section( 'main_styles', __( 'Main Style', 'radium' ), $options['main_styles_options'], 3 );
		...
		if( is_admin() ) radium_add_customizer_section( 'post_styles', __( 'Post Style', 'radium' ), $options['post_styles_options'], 4 );
		...
		if( is_admin() ) radium_add_customizer_section( 'ratings_styles', __( 'Rating System Style', 'radium' ), $options['ratings_styles_options'], 5 );
		...
		if( is_admin() ) radium_add_customizer_section( 'footer_styles', __( 'Footer Style', 'radium' ), $options['footer_styles_options'], 6 );
