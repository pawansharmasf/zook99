...
function radium_site_header_title() {
	...
	//* Build the title
	$output  = '<div class="site-title" itemprop="headline"><a href="'.trailingslashit( home_url() ).'" title="'.esc_attr( get_bloginfo( 'name' ) ).'" rel="home">';
	...
	if ( $logo ) {
		...
		if ( !$width & !$height ) {
			echo "<div class='alert error'>". __('Please setup the logo dimensions correctly in the theme options', 'radium') ."</div>";
		}
...
function radium_main_menu( ) {
	...
	if ( !$framework->theme_supports( 'primary', 'megamenu') ) return; ?>
